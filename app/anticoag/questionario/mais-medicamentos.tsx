...
import { gerarRecomendacaoItemAntiCoag } from "@/anticoag/logic/regrasConsolidado";
...
const ROTULO_CLASSE: Record<string, string> = {
  ...
  antiplaquetario: "Antiplaquetário",
...
export default function TelaMaisMedicamentos() {
  ...
  function adicionarOutro() {
    ...
    // classes adicionadas.
    router.replace("/anticoag/questionario/classe");
  }
  ...
  return (
    <ScrollView contentContainerStyle={estilos.container}>
      ...
      {respostas.medicamentos.map((item) => {
        const recomendacao = gerarRecomendacaoItemAntiCoag(item);
        return (
          <Cartao key={item.id} style={estilos.itemLista}>
            <View style={estilos.itemTexto}>
              <Text style={estilos.itemNome}>
                {recomendacao.medicamentoNome ?? "Medicamento"}
              </Text>
              <Text style={estilos.itemSubclasse}>{ROTULO_CLASSE[item.classe]}</Text>
            </View>
            <Botao
              titulo="Remover"
              variante="secundario"
